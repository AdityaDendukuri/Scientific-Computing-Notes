...
::: {.callout-note icon=false}
## Definition: Jacobi Eigenvalue Algorithm
**Input:**  Symmetric matrix $\bA\in\fR^{n\times n}$ and tolerance `tol`.\\
**Initialize:**  $\bA^{(0)}=\bA$, $\bV^{(0)}=\bI$.\\
**Repeat:**

1. Choose an off-diagonal pivot $(p,q)$ with $p<q$.
    
1. Compute $c=\cos\theta$ and $s=\sin\theta$ using the stable tangent formula so that $(\bG^T\bA^{(k)}\bG)_{pq}=0$.
    
1. Update the matrix by orthogonal similarity:
    $$
\begin{align}
\bA^{(k+1)}=\bG^T\bA^{(k)}\bG.
\end{align}
...
**Stop:**  when $\sigma_{\text{off}}(\bA^{(k)}) \leq `tol`\|\bA^{(k)}\|_F$.\\
**Output:**  Diagonal entries of $\bA^{(k)}$ approximate eigenvalues; columns of $\bV^{(k)}$ approximate eigenvectors.
:::
...
::: {.callout-tip icon=false}
## Remark
**(Why eigenvectors accumulate)**  After $k$ rotations,
$$
\begin{align}
\bA^{(k)} = (\bV^{(k)})^T \bA \bV^{(k)}.
\end{align}

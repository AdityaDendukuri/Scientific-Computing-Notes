...
## Definition: Stopping Tests for Root Finding
Common stopping tests are
$$
\begin{align}
|f(x_k)|\leq `tol`_f,
    \qquad
    |x_{k+1}-x_k|\leq `tol`_x(1+|x_k|).
\end{align}
$$
...
1. Rewrite Newton's method as a fixed-point iteration.
    
1. Compute $g'(x^*)$ for Newton's fixed-point map at a simple root.
...
::: {.callout-note icon=false}
## Theorem: Newton on a Multiple Root
...
1. Substitute $f$ and $f'$ into the Newton update.
    
1. Keep the leading term as $x_k\to x^*$.
    
1. Derive the factor in the result above.
...
\begin{tabular}{ll}
**Available information** & **Typical method** \\ \hline
...
1. Prove that for $f(x) = x^2$, Newton converges to $0$ with linear rate $1/2$ using the result above.
    
1. Find the root of $e^x = 3x$ near $x=1$ via Secant method. Compare evaluations vs.\ Newton.
    
1. Construct an example where $|f(x_k)|$ is small but $|x_k-x^*|$ is not small. Relate it to the result above.

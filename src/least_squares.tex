...
\addThm{Minimum-Norm Least Squares}{%
...
\label{thm:min-norm-least-squares}

\addExcr{%
\begin{enumerate}
    \item Write $\bA=\bU\bsigma\bV^T$.
    \item Transform the least squares problem into SVD coordinates.
    \item Identify which coordinates are determined by nonzero singular values.
    \item Show that setting nullspace coordinates to zero gives the minimum-norm solution in Thm~\ref{thm:min-norm-least-squares}.
\end{enumerate}
}
\label{ex:min-norm-least-squares}
...
\label{def:weighted-least-squares}

\addThm{Weighted Normal Equations}{%
The weighted least squares problem in Def~\ref{def:weighted-least-squares} satisfies
\begin{align*}
    \bA^T\mathbf{W}\bA\hat{\bx}=\bA^T\mathbf{W}\bb.
\end{align*}
}
\label{thm:weighted-normal-equations}

\addPrf{%
Apply the ordinary normal equations from Thm~\ref{thm:normal-equations} to the transformed system
\begin{align*}
...
\begin{enumerate}
    ...
    \item Derive Thm~\ref{thm:weighted-normal-equations} by differentiating the weighted objective directly.
\end{enumerate}
}

...
## Remark
**(Assumption)**  CG requires $\bA$ to be symmetric positive definite. If $\bA$ is nonsymmetric, use GMRES or another nonsymmetric Krylov method. If $\bA$ is symmetric indefinite, use methods designed for indefinite systems, such as MINRES.
:::
...
::: {.callout-tip icon=false}
## Remark
**(Residuals vs.\ directions)**  In exact arithmetic, CG residuals are mutually orthogonal in the Euclidean inner product, while search directions are mutually orthogonal in the $\bA$-inner product. These two orthogonality structures are what make the short recurrence possible.
:::


::: {.callout-tip icon=false}
## Remark
**Why not steepest descent?**  Steepest descent always moves in the negative gradient direction, but successive gradients can undo previous progress by zig-zagging across narrow energy valleys. CG corrects this by choosing conjugate directions adapted to the geometry of $\bA$.
:::
...
::: {.callout-tip icon=false}
## Remark
**(Practical stopping)**  A common stopping rule is
$$
\begin{align}
\frac{\|\br^{(k)}\|_2}{\|\bb\|_2} \leq `tol`.
\end{align}
$$
...
::: {.callout-tip icon=false}
## Remark
**(Polynomial view)**  CG chooses a degree-$k$ polynomial $p_k$ with $p_k(0)=1$ so that
$$
\begin{align}
\be^{(k)} = p_k(\bA)\be^{(0)}
\end{align}
...
The convergence of CG depends on the condition number $\kappa(\bA)$. **Preconditioning** transforms the system using a symmetric positive definite matrix $\bM$ that approximates $\bA$ but is much easier to invert.
...
::: {.callout-tip icon=false}
## Remark
**(Preconditioning is not optional in hard problems)**  For large PDE systems, unpreconditioned CG can require too many iterations. The preconditioner is often the main algorithmic design choice.
:::
